...
freqCoef2 = product2.sum(axis = 1);
freqPow2 = numpy.absolute(freqCoef2) ** 2;

f_known = numpy.fft.fft(freqCoef);
f_signal = numpy.fft.fft(freqCoef2);
f_corr = f_known.conj() * f_signal;
corr = numpy.fft.ifft(f_corr);
peak_loc = numpy.absolute(corr).argmax();
peak = numpy.absolute(corr[peak_loc]);
snr = peak / numpy.absolute(corr.mean());
print("Peak at {:+d} steps".format(peak_loc));
...
pp.figure();
pp.title("Cross-Correlation");
pp.xlabel("Note Steps");
pp.ylabel("Cross-Correlation Magnitude");
pp.plot(noteList, numpy.absolute(corr));
